...
let MAX_UPLOAD_FILE_SIZE_IN_BYTES = 190 * 1024 * 1024; // 190MB = 199229440B
...
Confluence.prototype.uploadOrUpdateFile = function (pageTitleOrId, fileName, filePath, callback) {
	...
	let fileSize = (jetpack.inspect(filePath) || {}).size || 0;

	if (fileSize > MAX_UPLOAD_FILE_SIZE_IN_BYTES) {
		Logger.log(`Skipping file '${fileName}'. Max file size exceeded!`);
		callback.call(me, {
			title: fileName
		});
	}
