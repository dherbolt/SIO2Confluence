...
let title2idFileName = APP_ROOT + '/data/title2id.json';
let filesInPageFileName = APP_ROOT + '/data/filesInPage.json';
	...
	const uploadFile = function(pageId, callback) {
		me.confluence.createAttachment(me.space, pageId, filePath, function(err, result) {
			if (err) {
				...
				this.filesInPage[pageId] = this.filesInPage[pageId] || [];
				this.filesInPage[pageId].push(attachment);
				jetpack.write(filesInPageFileName, JSON.stringify(this.filesInPage, null, '\t'));
